...
function Strategy(options, verify) {
	...
	options.authorizationURL =
		options.authorizationURL || 'https://oapi.dingtalk.com/connect/qrconnect';
	...
	if (!options.customHeaders['User-Agent']) {
		options.customHeaders['User-Agent'] =
			options.userAgent || 'passport-dingding';
	}

	OAuth2Strategy.call(this, options, verify);
	this.name = 'dingtalk';
	this._oauth2._userProfileURL =
		options.userProfileURL || 'https://oapi.dingtalk.com/sns/getuserinfo';
	this._oauth2._snsTokenURL =
		options.snsTokenURL || 'https://oapi.dingtalk.com/sns/get_sns_token';
	...
	this._oauth2.getAuthorizeUrl = function (params) {
		...
		let { appid, scope, redirect_uri, state } = params;
		let tmpParams = {
			appid,
			scope,
			state,
	...
	this._oauth2.getOAuthAccessToken = function (code, params, callback) {
		var self = this;
		urllib.request(
			this._persistentTokenURL + '?access_token=' + this.access_token,
			{
				headers: {
					'Content-Type': 'application/json',
				},
			...
			function (err, data, res) {
				...
				urllib.request(
					...
					function (err, data, res) {
						if (err) return callback(err);
						if (!data.sns_token)
							return callback(new Error('get sns_token failed'));
...
Strategy.prototype.userProfile = function (params, done) {
	urllib.request(
		this._oauth2._userProfileURL + '?sns_token=' + params.sns_token,
		{
			headers: {
				'Content-Type': 'application/json',
			},
